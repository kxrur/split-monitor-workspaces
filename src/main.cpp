...
static int getDelta(const std::string& direction)
{
    ...
    try {
        ...
        Log::logger->log(Log::WARN, "[split-monitor-workspaces] Invalid direction value: {}", direction.c_str());
    }
...
{
    Log::logger->log(Log::DEBUG, "[split-monitor-workspaces] Switch to workspace {}", workspace);
    ...
    if (absolute_workspace == std::to_string(getCurrentMonitor()->activeWorkspaceID())) {
        Log::logger->log(Log::DEBUG, "[split-monitor-workspaces] Next monitor ");
        absolute_workspace = std::to_string(getNextMonitor()->activeWorkspaceID());
    }

    Log::logger->log(Log::DEBUG, "[split-monitor-workspaces] Absolute workspace: {} ", absolute_workspace);
...
static void mapMonitor(const PHLMONITOR& monitor) // NOLINT(readability-convert-member-functions-to-static)
{
    ...
    Log::logger->log(Log::INFO, "{}",
                     "[split-monitor-workspaces] Mapping workspaces " + std::to_string(workspaceIndex) + "-" + std::to_string(workspaceIndex + g_workspaceCount - 1) + " to monitor " +
                         monitor->m_name);
...
static void unmapMonitor(const PHLMONITOR& monitor)
{
    ...
    Log::logger->log(Log::INFO, "{}",
                     "[split-monitor-workspaces] Unmapping workspaces " + std::to_string(workspaceIndex) + "-" + std::to_string(workspaceIndex + g_workspaceCount - 1) + " from monitor " +
                         monitor->m_name);
